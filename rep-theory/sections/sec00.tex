...
\begin{defn}%[Adjoint of an operator]
	Let $(V, \langle \cdot, \cdot\rangle)$ be an inner product space and $T \in \End(V).$ The \deff{adjoint of $T$} is the unique linear operator $T^*$ such that the following equality holds for all $v, w \in V:$
	\begin{equation*} 
...
\begin{defn}%[Unitary matrix] 
	...
	A matrix $U \in \GL_n(\mathbb{C})$ is said to be \deff{unitary} if $UU^* = I.$ The set of all such matrices is denoted by $U_n(\mathbb{C})$ and forms a subgroup of $\GL_n(\mathbb{C}).$
\end{defn}
...
\begin{defn}%[Minimal polynomial]
	Let $T \in \End(V).$ The \deff{minimal polynomial} of $T$ is the unique monic polynomial $m(X) \in \mathbb{C}[X]$ of least degree such that $m(T)$ is the zero operator.
\end{defn} 
...
Recall that given a group $G$ and elements $a, b \in G$, we say that $a$ and $b$ are \deff{conjugates} if there exists $g \in G$ such that $a = gbg^{-1}$.
...
\end{prop}
\begin{proof} 
	Let $\alpha = p/q \in \mathbb{Q}$ be an algebraic integer with $p \in \mathbb{Z}, q \in \mathbb{N}$ and $\gcd(p, q) = 1.$ Since $\alpha$ is an algebraic integer, there exist $a_0, \ldots, a_{n-1}$ such that
	\begin{equation*} 
		\alpha^n + a_{n - 1}\alpha^{n - 1} + \cdots + a_0 = 0.
	\end{equation*}
	...
	\end{equation*}
	Clearly, every term except for the first is divisible by $q.$ Thus, so is the first term. That is, $q \mid p^n.$ Since $\gcd(p, q) = 1$ and $q > 0,$ we get $q = 1.$ Thus, $\alpha = p \in \mathbb{Z},$ as desired.
\end{proof}
...
\begin{prop} \label{prop:characalgint}
	...
	\begin{equation*} 
		...
		\end{bmatrix}.
	\end{equation*}
	In other words, each $yy_i$ is an integral linear combination of the $y_j.$ In yet other words, $y$ is an eigenvalue of an integer matrix.
\end{prop}
\begin{proof} 
