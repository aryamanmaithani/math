...
\begin{cor} \label{cor:finabelrepdiagonal}
	Let $G$ be a finite abelian group and $\varphi : G \to \GL_n(\mathbb{C})$ a representation. Then, there exists an invertible matrix $T$ such that $T^{-1}\varphi_gT$ is invertible all $g \in G.$
\end{cor}
...
\begin{proof} 
	...
	\begin{equation*} 
		\varphi_g = T\rho_gT^{-1}
	\end{equation*}
	for all $g \in G.$ Now fix $g \in G$ and using $\trace(ABC) = \trace(CAB),$ we see that
	\begin{align*} 
