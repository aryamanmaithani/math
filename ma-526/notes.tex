...
\def\imagename{icon.pdf}		    % Replace with University Seal
...
\end{defn}
\begin{defn}%[]
	Let $M$ be an $R$-module and $x \in M.$ If $\mathfrak{p} = 0:_Rx$ is a prime in $R,$ then we say that $\mathfrak{p}$ is an \deff{associated prime} of $M.$
	\begin{equation*} 
...
\end{defn}
\begin{defn}%[]
	Fix $x \in M.$ The map $\mu_x : R \to M$ defined by $r \mapsto rx$ is called the \deff{homothety} by $x.$
\end{defn}
...
\begin{defn}%[]
	$a \in R$ is a \deff{zerodivisor} on $M$ if $ax = 0$ for some $0 \neq x \in M.$
	\begin{equation*} 
		\mathcal{Z}(M) \vcentcolon= \{a \in R \mid a \text{ is a zerodivisor on }M\}.
	\end{equation*} 
	If $a$ is not a zerodivisor, then $a$ is called a \deff{nonzerodivisor} on $M$ or \deff{$M$-regular}.
\end{defn}
...
\begin{defn}%[]
	Let $R \subset S$ be nonzero commutative rings. An element $s \in S$ is called \deff{integral} over $R$ if there exists a \underline{monic} polynomial $f(x) \in R[x]$ such that $f(s) = 0.$
