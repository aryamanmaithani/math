...
\begin{exe}
    Call a polynomial $f(x_{1}, \ldots, x_{n}) \in \mathbb{F}[x_{1}, \ldots, x_{n}]$ \deff{alternating} if swapping two of the arguments changes the polynomial by a sign. \\
    Define 
    \begin{equation*} 
        V_{n} \vcentcolon= \prod_{1 \le i < j \le n} (x_{i} - x_{j}).
    \end{equation*}
    Show that $V_{n}$ is alternating. \\
    Given an alternating polynomial $f(x_{1}, \ldots, x_{n})$, show that there exists a unique symmetric polynomial $g(x_{1}, \ldots, x_{n})$ such that
    \begin{equation*} 
        f(x_{1}, \ldots, x_{n}) = V_{n} \cdot g(x_{1}, \ldots, x_{n}).
    \end{equation*}
    (Hint: Show that $x_{i} - x_{j}$ divides $f$ for all $1 \le i < j \le n$.)
\end{exe}
...
\begin{rem}
    ...
    An alternate (but longer) definition could have been to start with $f(x)  = x^n - a_1x^{n - 1} + \cdots + (-1)^n a_n \in \mathbb{F}[x]$ and define 
    \begin{equation*} 
...
\label{lem:FTAprelim}
    ...
    \begin{enumerate}
        ...
        \item Every complex number has a square root. Thus, every complex quadratic polynomial has all roots in $\mathbb{C}.$ \hfill\hyperref[lem:FTAprelim2]{\downsym}
    \end{enumerate} 
\end{restatable}
