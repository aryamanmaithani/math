...
\begin{defn}%[]
    Let $\mathbb{K}/\mathbb{F}$ be a field extension. Then, we may regard $\mathbb{K}$ as a vector space over $\mathbb{F}.$ We denote $\dim_{\mathbb{F}}\mathbb{K}$ by $[K : F]$ and call it the \deff{degree} of the field extension $\mathbb{K}/\mathbb{F}.$
\end{defn}
...
\begin{prop} \label{prop:decompalgisalg}
    ...
    If $\alpha$ is algebraic over $\mathbb{F},$ then $\alpha$ is algebra over $\mathbb{E}.$ \\
