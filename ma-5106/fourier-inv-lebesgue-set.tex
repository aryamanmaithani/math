...
\begin{frame}{The $A$ Game}
    First, we note that
    \begin{flalign*} 
         && \vphantom{\left(\int_{r/2 \le \md{x} \le r}^{} 1\right)} \only<3>{\int_{r/2 \le \md{x} \le r}^{} \psi_0(r) \,{\mathrm{d}}u}\only<4>{\left(\int_{r/2 \le \md{x} \le r}^{} 1\right)}\only<5->{V_n\left(1 - \frac{1}{2^n}\right)r^n}\only<4->{\psi_0(r)}\uncover<3->{\le}\uncover<2->{\int_{r/2 \le \md{u} \le r}^{} \varphi(u) \,{\mathrm{d}}u}\uncover<6->{\to 0, } \qquad \hphantom{hi}
    \end{flalign*}
