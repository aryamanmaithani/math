...
\begin{frame}{Proof of the Main Theorem}
    ...
    \begin{equation} \tag{$L$} \label{eq:02}
        \only<4->{\phantom{\frac{1}{r^n}}}\only<3>{\frac{1}{r^n}}\int_{\md{t} < r}^{} \md{f(x - u) - f(x)} \,{\mathrm{d}}u < \epsilon \only<4->{r^n}\only<3>{\phantom{r^n}}
    \end{equation} }\uncover<5->{for all $0 < r \le \delta.$ }
