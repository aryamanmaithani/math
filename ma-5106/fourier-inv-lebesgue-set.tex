...
\newcommand{\myvphan}{\vphantom{\md{\int_{\md{u} < \delta}^{} [f(x - u) - f(x)]\varphi_t(u) \,{\mathrm{d}}u}}}
...
\begin{frame}{The proof from class}
    \uncover<1->{We had seen that $\{h_t\}_{t > 0}$ constitutes a (continuous) approximate identity. } \uncover<2->{We thus concluded that $f * h_t \xrightarrow{t \to 0} f$ in $L^1$ for any $f \in L^1.$ }\uncover<3->{In turn, that gave us that 
    ...
    \begin{enumerate}
        ...
        \uncover<6->{\item Using DCT, we let $t \to 0$ in the above to conclude that
        \begin{equation*} 
...
\begin{frame}{Conclusion}
    ...
    \uncover<4->{Our aim now is to show that (\ref{eq:01}) holds for all $x$ in the \emph{Lebesgue set} of $f.$ }

    \uncover<5->{We will actually prove the result for a broader class of approximate identities. }
\end{frame}
...
\begin{frame}{Lebesgue set}
    ...
    \begin{defn}[Lebesgue set]
        ...
        \begin{equation*} 
            \lim_{r\to 0}\frac{1}{r^n}\int_{\md{u} < r}^{} \md{f(x - u) - f(x)} \,{\mathrm{d}}u = 0
...
\begin{frame}{The identity}
    \uncover<1->{Let $\varphi \in L^1(\mathbb{R}^n)$ be a \underline{radial} function with $\|\varphi\|_1 = 1.$ }\uncover<2->{Let $\psi_0 : \mathbb{R} \to \mathbb{R}$ be defined as }\uncover<3->{
    \begin{equation*} 
        \psi_0(\|x\|) = \varphi(x)
...
\end{frame}
\begin{frame}{The identity theorem}
...
\begin{frame}{Proof of the Main Theorem}
    ...
    \begin{equation} \tag{$L$} \label{eq:02}
        \int_{\md{t} < r}^{} \md{f(x - u) - f(x)} \,{\mathrm{d}}u < \epsilon r^n
    \end{equation} }\uncover<4->{for all $0 < r \le \delta.$ }
    
    \uncover<5->{Note that for all $t > 0,$ we have $\displaystyle\int_{\mathbb{R}^n} \varphi_t = \int_{\mathbb{R}^n} \varphi = \int_{\mathbb{R}^n} \md{\varphi} = 1.$ }
\end{frame}
\begin{frame}{Still proving the Main Theorem}
    ...
    \uncover<3->{\begin{align*} 
        ...
        \uncover<7->{&\le \md{\int_{\md{u} < \delta}^{} [f(x - u) - f(x)]\varphi_t(u) \,{\mathrm{d}}u} \uncover<8->{{\color{blue}\leadsto I_1\uncover<9->{(t)}} } \\
        &\qquad + \md{\int_{\md{u} \ge \delta}^{} [f(x - u) - f(x)]\varphi_t(u) \,{\mathrm{d}}u} \uncover<8->{{\color{blue}\leadsto I_2\uncover<9->{(t)}} } }
    \end{align*} }
\end{frame}
\begin{frame}{The $A$ Game}
    First, we note that
    \begin{flalign*} 
         && \vphantom{\left(\int_{r/2 \le \md{x} \le r}^{} 1\right)} \only<3>{\int_{r/2 \le \md{x} \le r}^{} \psi_0(r) \,{\mathrm{d}}u}\only<4>{\left(\int_{r/2 \le \md{x} \le r}^{} 1\right)}\only<5->{V_n\left(1 - \frac{1}{2^n}\right)r^n}\only<4->{\psi_0(r)}\uncover<3->{\le}\uncover<2->{\int_{r/2 \le \md{u} \le r}^{} \varphi(u) \,{\mathrm{d}}u}\uncover<6->{\to 0, }
    \end{flalign*}
...
\begin{frame}{Taking $I_2$ down}
    \begin{flalign*} 
        I_2(t) \only<2>{= \md{\int_{\md{u} \ge \delta}^{} [f(x - u) - f(x)]\varphi_t(u) \,{\mathrm{d}}u}} %
        \only<3->{\le \int_{\md{u} \ge \delta}\md{f(x - u)}\varphi_t(u)\,{\mathrm{d}}u + \md{f(x)}\int_{\md{u} \ge \delta}\varphi_t(u)\,{\mathrm{d}}u.} \vphantom{\md{\int_{\md{u \ge \delta}}^{\infty}  \,{\mathrm{d}}x}} &&
    \end{flalign*}
    \uncover<4->{The second term goes to $0$ as $t \to 0$ }\uncover<5->{since $\{\varphi_t\}_{t > 0}$ is an approximate identity. }

    \uncover<6->{Now, let $\chi_\delta$ denote the characteristic function of $\{u \in \mathbb{R}^n : \md{u} \ge \delta\}.$ }
    ...
    \begin{equation*} 
        \|\chi_\delta \varphi_t\|_{\infty} = \sup_{\|u\| \ge \delta}\only<9>{\varphi_t(u)\hphantom{t^{-n}}}\only<10->{t^{-n}\varphi(u/t)} \only<11>{= t^{-n}\psi_0(\delta/t)\qquad}\only<12->{=\delta^{-n}(\delta/t)^n\psi_0(\delta/t)}\uncover<13->{\to 0,}
    \end{equation*} }
...
\begin{frame}{Some more calculations}
    We have
    \begin{flalign*} 
        \uncover<2->{I_1(t)&}\only<2>{=}\only<2>{\md{\int_{\md{u} < \delta}^{} [f(x - u) - f(x)]\varphi_t(u) \,{\mathrm{d}}u} } %
        \only<3->{\le}\only<3>{\int_{\md{u} < \delta}^{} \md{f(x - u) - f(x)}\varphi_t(u) \,{\mathrm{d}}u \myvphan}%
        \only<4->{\int_{\md{u} < \delta}^{} \md{f(x - u) - f(x)}t^{-n}\varphi(u/t) \,{\mathrm{d}}u \myvphan} && \\
...
\begin{frame}{The Green Integral}
    ...
    \begin{align*} 
        ...
        &\only<4>{=0 + \frac{n}{\omega\left(S^{n - 1}\right)}\int_{\mathbb{R}^n}^{} \varphi }\only<5->{=\frac{n}{\omega\left(S^{n - 1}\right)}.}
    \end{align*}
...
\begin{frame}{Concluding Remark}
    ...
    \uncover<3->{\begin{thm}[General Theorem]
        Suppose $\varphi \in L^1(\mathbb{R}^n).$ \uncover<4->{Let $\psi(y) = \displaystyle\esssup_{\|z\| \ge \|y\|}\varphi(z)$ }\uncover<5->{and for $t > 0,$ let $\varphi_t(y) = t^{-n}\varphi(y/t).$ }\uncover<6->{If $\psi \in L^1(\mathbb{R}^n)$ and $f \in L^p(\mathbb{R}^n),$ $1 \le p \le \infty,$ }\uncover<7->{then $\displaystyle\lim_{t\to 0}(f * \varphi_t)(x) = f(x)\int_{\mathbb{R}^n}^{} \varphi(t) \,{\mathrm{d}}t$ }\uncover<8->{whenever $x \in \Leb(f).$ }
    \end{thm} }
