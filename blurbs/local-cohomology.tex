...
\end{defn}
Minimal injective resolutions always exist over any ring (this amounts to showing that every module does have an injective hull, which amounts to showing that every module embeds into an injective module and then using \Cref{prop:maximal-essential-inside-injective}). Any two such resolutions are isomorphic as complexes.
...
\begin{thm}
	\Cref{eq:001} is an isomorphism. In particular, $\widehat{R} \cong E^{\vee}$. In turn, if $R$ is a complete local ring, then the obvious map $R \to E^{\vee}$ is an isomorphism.
\end{thm}
...
\begin{thm}[Matlis duality between Ext and Tor]
...
\begin{obs}
	...
	Next, suppose $(I_{t})_{t}$ and $(J_{t})_{t}$ are decreasing sequences of ideals of $R$ which are \deff{cofinal}, i.e., for all $t$, there exist $u$ and $v$ such that $J_{u} \subset I_{t}$ and $I_{v} \subset J_{t}$. Then, the direct limit computed using either sequence is the same. Indeed, we can form a sequence
	\begin{equation*} 
...
\begin{thm}
	...
	\end{equation}
\end{thm}
Recall: $\depth_{I} M$ is the length of the longest sequence in $I$ which is regular on $M$. Moreover, it is customary to define $\depth_{I}(M) \vcentcolon= \infty$ if $IM = M$; consistent with \Cref{eq:002}.
...
\begin{itemize}
	...
	\item take the cohomology of each object in \Cref{eq:004} to get a direct system $\cdots \to H^{\bullet}(\mathbf{x}^{t}; M) \to H^{\bullet}(\mathbf{x}^{t + 1}; M) \to \cdots$ and then take its limit.
\end{itemize}
