...
\Cref{part:definitions,part:theorems} are definitions and theorems about general noncommutative algebra. 
We first begin by stating all the relevant definitions in \Cref{part:definitions}, 
followed by the theorems in \Cref{part:theorems}. 

\Cref{part:rep-th} focuses on facts about the group ring $k[G]$. 
...
\begin{defn}
	A surjection $f \colon M \onto N$ is called an \deff{essential surjection} if 
...
\begin{defn}
	$A$ is \deff{quasi-Frobenius} if $R$ is left noetherian and injective as a left module over itself.
\end{defn}
This turns out to be highly symmetric: see \Cref{thm:quasi-frobenius-equivalences}.

\begin{defn}
	A \deff{Frobenius algebra} over a field $k$ is a finite-dimensional $k$-algebra $A$ equipped with 
...
\begin{defn}
	An element $e \in A$ is called \deff{idempotent} if $e^{2} = e$. 
	Two idempotents $e$ and $f$ are called \deff{orthogonal} if $ef = fe = 0$. 
	The idempotents $0$ and $1$ are the \deff{trivial idempotents}.
	The idempotent $e$ is a \deff{primitive idempotent} if $e$ cannot be written as a sum of two nonzero orthogonal idempotents.

	If $I$ is an ideal of $A$, then an idempotent $e \in A/I$ is said to \deff{lift modulo $I$} if there exists an idempotent $f \in B$ with $e = f + I$.
\end{defn}
...
\begin{thm}[Krull--Remak--Schmidt] \label{thm:krs}
	Let $M$ be an $A$-module of finite length. 
...
\begin{thm}
	...
	\artinianimplications{orange!15}
\end{thm}
...
\end{thm}
\begin{sketch}
	Follows from \mynameref{thm:krs}.
\end{sketch}
\begin{thm}
...
\begin{defn}
	The \deff{group ring} (or the \deff{group algebra}) $k[G]$ is the $k$-algebra defined as follows:
	\begin{itemize}
...
\begin{defn}
	The algebra $k[G]$ is further an \deff{augmented algebra}.

	The \deff{augmentation map} is defined as
	\begin{align*} 
	...
	The \deff{augmentation ideal} of $G$ is defined as
	\begin{equation*} 
...
This is called the \deff{diagonal homomorphism} (or \deff{coproduct}) of the group algebra $k[G]$.
...
\end{equation*}
This map is called the \deff{antipode}.
...
\end{thm}
\begin{sketch}
	% (b) $\Rightarrow$ (a): Suppose $\chr(k) \mid \md{G}$. 
	...
	(b) $\Leftrightarrow$ (c): follows from \Cref{thm:quasi-frobenius-either-semisimple-or-infinite} since $k[G]$ is quasi-Frobenius.
...
Suppose $p = 3$: By \Cref{thm:facts-about-kG}, we know that $k\Sigma_{3}$ is a sum of projective indecomposables and 
...
Recall that a group algebra is a \deff{supplemented algebra}, i.e., 
...
The \deff{cohomology of $R$ with coefficients in $M$} is the graded $k$-vector space $\Ext_{R}^{\ast}(k, M)$. \newline
When $M = k$, we get $\Ext_{R}^{\ast}(k, k)$, which is usually called the \deff{cohomology of $R$}.
...
called the \deff{endomorphism DGA} of $P$.
