...
\begin{enumerate}[label=(\alph*),leftmargin=0em]
	\item \label{item:reynolds-splitting} If $G$ is a finite group and $\frac{1}{\md{G}} \in K$, then there exists a splitting $\frac{1}{\md{G}} \sum_{g \in G} g : S \to S^{G}$. Thus, the inclusion is pure. The splitting is often called the \deff{Reynold's operator}.
	%
