...
\begin{proof} 
	...
	By \Cref{exe:grade-Hom-min-2-grade}, we have
...
\begin{proof} 
	...
	\backward By \Cref{thm:projective-locally-free}, we first see that $M$ is projective. Now, (c) of \Cref{prop:rank-equivalences} tells us that $M$ has a rank and that this rank is $r$.
\end{proof}
...
\begin{defn}
	...
	\end{equation*}
	the \deff{$k$-th Fitting invariant of $M$} is defined to be $I_{n - k}(\varphi)$, and is denoted by $\Fit_{k}(M)$.
\end{defn}
...
Combining the above with \Cref{cor:projective-rank-iff-locally}, we get the following global version.
...
\begin{proof} 
	...
	\begin{enumerate}[label=(\alph*)]
		...
		\item $I_{r}(\varphi) = R$ and $I_{r + 1}(\varphi) = 0$.
	\end{enumerate}
	But this now follows from \Cref{prop:projective-rank-using-presentation}.
\end{proof}
...
\begin{defn}
	...
	\end{equation*}
	of $R$-modules is called \deff{acyclic} if $H_{i}(G_{\bullet}) = 0$ for all $i > 0$, and \deff{split acyclic} if it is acyclic and $\psi_{i + 1}(G_{i + 1})$ is a direct summand of $G_{i}$ for $i \ge 0$.
\end{defn}
