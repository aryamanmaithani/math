\begin{tikzpicture}
	...
	\draw[very thick, red, ->-=at 0.25 with label {}] (0, 2*\len) -- (2*\len + \eps, 2*\len);
	\draw[very thick, blue] (2*\len + \eps, 2*\len) -- (3*\len + \eps, 2*\len) -- (3*\len+\eps, \len);
	\draw[very thick, red] (3*\len+\eps, \len) -- (4*\len, \len);
\end{tikzpicture}